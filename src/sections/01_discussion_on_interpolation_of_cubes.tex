...
the young explorer always possessed a strong sense of mathematical beauty and aesthetics.
The student's mathematical knowledge was limited to undergraduate-level courses, which included the basics of
matrix operations, basic calculus, and elementary linear algebra.
One day, the student found himself observing tables of finite differences,
in particular, finite differences of cubes.
That was the year 2014.

\input{sections/figures/01_fig_finite_differences_cubes}
Thus, the following problem was stated
\begin{problem}
...
The problem of interpolation is quite old and can be traced back
...
Interpolation, as we know it today, has been developed between 1674--1684 by Sir Isaac Newton.
...
\end{align*}
where $6,6,1,0$ correspond to the first row in table~\eqref{tab:finite-differences-cubes}.
In general,
\begin{align*}
...
Great!
But there is one thing: the student who stated the problem~\eqref{quest:interpolation-cubes}
...
The student's mind was occupied by only one thought:
\textit{All mathematical truths exist timelessly, we only reveal and describe them.}
That mindset inspired our young researcher to start his own mathematical journey.

By observing the finite differences from table~\eqref{tab:finite-differences-cubes}, we can notice that
...
\begin{align*}
    \Sigma^{r} n^3 &=  6 \binom{n+1+r}{3+r} + \binom{n+r}{1+r} \\
    \Delta^{r} n^5 &= 120 \binom{n+2-r}{5-r} + 30 \binom{n+1-r}{3-r} + \binom{n-r}{1-r}
\end{align*}
...
our protégée's problem~\eqref{quest:interpolation-cubes} was successfully solved,
because we have found a formula that recovers the cubes $n^3$ from their finite differences.
It is indeed straightforward that $5^3 = \sum_{k=0}^{4} 1 + 3 k + 3 k^2 = 1 + 7 + 19 + 37 + 61 = 125$ which is exactly
the sum of terms of the first column of table~\eqref{tab:finite-differences-cubes}.

However, not this time.
Luckily enough (say), the student who stated the problem~\eqref{quest:interpolation-cubes}
was not really aware of the approaches above either.
What a lazy student!
Probably, that is exactly the case when unawareness leads to a fresh sight into classical problems,
leading to unexpected results and new insights.
Instead, our investigator observed a different pattern in
...
\end{align*}
By arranging the values of $T_{1} (n,k)$ in a table, we see that the identities above are indeed true.
...
However, the excitement shall not last too long.
Indeed, curiosity is not the thing that can be fulfilled completely,
...
Moreover, it was not even clear what is the general form of polynomial identity to search for.
