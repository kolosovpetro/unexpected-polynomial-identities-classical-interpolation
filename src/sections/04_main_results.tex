Thus, the conjecture~\eqref{conj:odd-power-identity} is true
\begin{theorem}[Odd power identity]
    \label{theorem:odd-power-identity}
    For non-negative integers $m$ and $n \geq 1$,
    there is a set of coefficients $\coeffA{m}{0}, \coeffA{m}{1}, \ldots, \coeffA{m}{m}$ such that
    \begin{align*}
        n^{2m+1} = \sum_{r=0}^{m} \coeffA{m}{r} \sum_{k=1}^{n} k^r (n-k)^r
    \end{align*}
\end{theorem}
Note that the polynomial $\sum_{k=1}^{n} k^r (n-k)^r$ is a polynomial of degree $2r+1$ in $n$.

In explicit form
\begin{align*}
    n^{2m+1}
    &= \sum_{r=0}^{m} \sum_{k=1}^{n} \coeffA{m}{r} k^r (n-k)^r = \sum_{r=0}^{m} \sum_{k=1}^{n} \coeffA{m}{r} (kn-k^2)^r \\
    &= \sum_{r=0}^{m} \coeffA{m}{r} \left[ 1^r \cdot (n-1)^r + 2^r \cdot (n-2)^r + 3^r \cdot (n-3)^r + \cdots + n^r \cdot (n-n)^r  \right] \\
    &= \sum_{r=0}^{m} \coeffA{m}{r} \left[ (n-1)^r + (2n-4)^r + (3n-9)^r + (4n-16)^r + \cdots +  (n^2-n^2)^r  \right]
\end{align*}
We define $x^0 = 1$ for all $x$, see~\cite[~p. 162]{graham1994concrete}.
This is because when $k=n$ and $r=0$ the term $k^r (n-k)^r = n^0 \cdot 0^0$, thus we define $x^0 = 1$
for all $x$.

\begin{definition}[Bivariate sum $T_m$]
    For integers $n,k$ and $m \geq 0$
    \label{def:bivariate-sum-Tm}
    \begin{align*}
        T_{m}(n,k) = \sum_{r=0}^{m} \coeffA{m}{r} k^r (n-k)^r
    \end{align*}
\end{definition}
The polynomial $T_m(n,k)$ is a polynomial of degree $m$ in $n$, and of degree $2m$ in $k$.
\begin{proposition}[Symmetry of $T_m$]
    \label{prop:Tm-symmetry}
    For integers $n, k$ and $m$
    \begin{align*}
        T_{m} (n, k) = T_{m} (n, n-k)
    \end{align*}
\end{proposition}

\input{sections/subsections/main-results/01_main_results_sums_of_odd_powers}
\input{sections/subsections/main-results/02_main_results_forward_decompositions}
\input{sections/subsections/main-results/03_main_results_forward_decompositions_multifold}
\input{sections/subsections/main-results/04_main_results_backward_decompositions}
\input{sections/subsections/main-results/05_main_results_backward_decompositions_multifold}
\input{sections/subsections/main-results/06_main_results_binomial_forms}
\input{sections/subsections/main-results/07_main_results_double_bivariate_sums}

