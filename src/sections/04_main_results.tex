...
We may see it by observing the Pascal's identity
$(n+1)^{k+1}-1=\sum _{p=0}^{k}{\binom {k+1}{p}}(1^{p}+2^{p}+\dots +n^{p})$.
In particular,
\begin{align*}
    n^{2k+1} - 1
    = \sum_{p=0}^{k} \coeffA{k}{p} \left[ (n-1)^p + (2n-4)^p + (3n-9)^p + (4n-16)^p + \cdots +  (n-1)^p  \right]
\end{align*}
