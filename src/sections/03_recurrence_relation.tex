...
The main idea of Alekseyev's approach was to utilize a certain generating function to evaluate
...
We use a specific version of Faulhaber's formula~\cite{beardon1996sums} with upper summation bound set to $p+1$
\begin{align*}
    \sum_{k=1}^{n} k^{p}
...
\end{align*}
The reason we use the Faulhaber's formula above is that we tend to omit summation bounds, for simplicity.
...
To simplify equation~\eqref{eq:rearranging-terms} using binomial identity~\eqref{lem:piecewise-binomial-identity},
we have to move $j=0$ out of the summation,
to avoid division by zero in $\frac{(-1)^r}{j}$.
Therefore,
\begin{equation*}
    \begin{split}
        \sum_{k=1}^{n} k^{r} (n-k)^{r}
...
\begin{proposition}[Bivariate Faulhaber's Formula]
    ...
    \end{align*}
\end{proposition}
Note that the polynomial $\sum_{k=1}^{n} k^r (n-k)^r$ is a polynomial of degree $2r+1$ in $n$.
...
\end{equation*}
Proceeding recursively, we can compute each coefficient $\coeffA{m}{r}$ for integers $r$ in the range
...
\end{equation*}
Finally, we define the following recurrence relation for the coefficients $\coeffA{m}{r}$
\begin{proposition}
