...
\begin{center}
    ...
    \begin{tabular}{ll}
        ...
        \texttt{BivariateSumT[m, n, k]}           & Bivariate sum $T_m(n, k)$ (Definition~\ref{def:bivariate-sum-Tm})  \\
        \texttt{TableFormBivariateSumT[m, rows]}  & Triangle view of $T_m(n, k)$                                       \\
        \texttt{T[d, r]}                          & Definition~\ref{def:polynomial-tdr}                                \\
        \bottomrule
    \end{tabular}
\end{center}
...
\begin{center}
    ...
    \begin{tabular}{ll}
        ...
        \texttt{TableFormForwardRecurrenceForT[m, rows]}          & Triangle view of~\ref{prop:Tm-recurrence-forward}                  \\
        \bottomrule
    \end{tabular}
\end{center}
...
\begin{center}
    ...
    \begin{tabular}{ll}
        ...
        \texttt{TableFormBackwardRecurrenceForT[m, rows]}          & Triangle view~\ref{prop:Tm-recurrence-backward}                 \\
        \bottomrule
    \end{tabular}
\end{center}
...
\textbf{Double bivariate identities}
\begin{center}
    \renewcommand{\arraystretch}{1.3}
    \begin{tabular}{ll}
        \toprule
        \textbf{Mathematica Function}                             & \textbf{Validates / Defines}                                        \\
        \midrule
        \texttt{DoubleBivariateSumR[m, n, t]}                     & Definition~\ref{def:double-bivariate-sum-r}                         \\
        \texttt{OddPowerDoubleBivariate[n, m]}                    & Proposition~\ref{prop:odd-power-double-bivariate}                   \\
        \texttt{OddPowerDoubleBivariateMultifold[n, m, s]}        & Proposition~\ref{prop:odd-power-double-bivariate-multifold}         \\
        \texttt{OddPowerDoubleBivariateNegated[n, m]}             & Proposition~\ref{prop:odd-power-double-bivariate-negated}           \\
        \texttt{OddPowerDoubleBivariateNegatedMultifold[n, m, s]} & Proposition~\ref{prop:odd-power-double-bivariate-negated-multifold} \\
        \bottomrule
    \end{tabular}
\end{center}
