...
These preprints discussed the cases of  $n^5, \; n^7$ and $n^9$ in context of problem~\eqref{question:higher_powers}.
Furthermore, these results were refined and published in \textit{Mathematical gazette}
...
\end{align*}
By observing the equation above, the general form of the odd-power identity becomes clearer.
We define $x^0 = 1$ for all $x$, see~\cite[~p. 162]{graham1994concrete},
because when $k=n$ and $r=0$ the term $k^r (n-k)^r = n^0 \cdot 0^0$, thus we define $x^0 = 1$
for all $x$.
...
\end{equation*}
By multiplying both sides by $30$, we get
\begin{equation*}
    30 \coeffA{2}{0} n + 5 \coeffA{2}{1} (n^3-n) + \coeffA{2}{2} (n^5-n) - 30n^5 = 0
\end{equation*}
Expanding the brackets and rearranging the terms yields
\begin{equation*}
...
\begin{align*}
    T_2 (n,k) = T_2 (n,n-k)
\end{align*}
By arranging the values of $T_{2} (n,k)$ in a table, we may observe that the identity for $n^5$ above is indeed true
...
Thus, our curious learner obtained the solution for the problem~\eqref{question:higher_powers}.
This time, the solution contained even more than a methodology to evaluate
the coefficients $\coeffA{2}{0}, \coeffA{2}{1}, \ldots, \coeffA{2}{2}$ ---
it revealed a general form of identity for odd powers.

Hence, let be a conjecture
\begin{conjecture}
    \label{conj:odd-power-identity}
    For non-negative integers $m$ and $n\geq1$,
    ...
    \end{align*}
\end{conjecture}
The polynomial $\sum_{k=1}^{n} k^r (n-k)^r$ is a polynomial of degree $2r+1$ in $n$.

As we already know, we have to build and solve a system of linear equations to evaluate the set of coefficients
$\coeffA{m}{0}, \coeffA{m}{1}, \ldots, \coeffA{m}{m}$.
However, the conjecture~\eqref{conj:odd-power-identity} cannot be proven by building and solving countless
...
Yet again, the intuition was correct.
